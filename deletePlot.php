...
if(isset($_GET['id'])){
    ...
    if($connect->query($queryPlanted) === TRUE){
        if($connect->query($sql) === TRUE){
            echo "<script> location.href='viewPlots.php'; </script>";
            echo "Deleted the data";
        }
        else{
            echo "<script> location.href='viewPlots.php'; alert('Error Deleting Plots')</script>";
        }
    }
    else{
        echo "<script> location.href='viewPlots.php'; alert('Error Deleting Plot Child Classes')</script>";
    }
}
else{
    die("ID Not provided");
