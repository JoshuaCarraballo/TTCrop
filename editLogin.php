<?php
    ...
    $id = $_GET['id'];
    $sql = "SELECT * FROM logindetails where loginID=$id";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['btnUpdate'])) {
            ...
            $role = $_POST["farmerRole"];
            $sql1= "UPDATE logindetails SET username='$username', password='$password', role='$role'
            WHERE loginID = '$id' ";
...
<body>
    ...
    <header>
        ...
        <main>
            ...
            <h2 class="dash-title">Edit User</h2>
...
<form class="form-control" method="POST" name="farmerForm">
    <div class="form-group">
      ...
      <?php
            $query = "SELECT farmerName FROM farmer where farmerID=$farmerID";
            $result2 = mysqli_query($connect,$query);
            $row2 = mysqli_fetch_array($result2);
            $farmerName = $row2[0];
        ?>
      <input type="text" class="form-control" id="farmerName" name="farmerName" value="<?php echo $farmerName; ?>" readonly>
    </div>
   <div class="form-row">
  ...
  <div class="form-group">
      ...
      <select class="form-control" name="farmerRole">
        <?php
            if($role=="Farm Owner"){
                echo '<option>Farm Owner</option>';
                echo '<option>Manager</option>';
                echo '<option>Regular Employee</option>';
            }
